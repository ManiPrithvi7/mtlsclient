...
- **Client identity (mTLS):** **`cert`** + **`key`** from **`src/crts/`** (see `device_keys_from_header.js` for filenames).
- **Trust the EMQX server cert:** If **`src/crts/broker-ca.crt`** exists, it is merged with the system CA store. **`MQTT_BROKER_CA=`** (empty) forces system trust only. **`MQTT_BROKER_CA=/path`** points at a specific PEM.
...
**mTLS is always used on `mqtts://` with `src/crts/` keys.** MQTT-layer credentials are **optional** and depend on how **EMQX** is configured:
...
| **`unable to verify …` (server cert)** | **`src/crts/broker-ca.crt`** or **`MQTT_BROKER_CA`**. |
...
| **`src/crts/`** | Device cert, key, optional **`broker-ca.crt`** |
